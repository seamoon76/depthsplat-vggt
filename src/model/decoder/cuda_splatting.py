...
def render_cuda_gsplat(
    ...
    scale_invariant: bool = True,
    use_sh: bool = True,
) -> tuple[Float[Tensor, "batch view 3 height width"], Float[Tensor, "batch view height width"]]: #Float[Tensor, "batch 3 height width"]:
    assert use_sh or gaussian_sh_coefficients.shape[-1] == 1
    ...
    for i in range(b):
        ...
        image = render_colors[..., :3].permute(0, 3, 1, 2).clip(0, 1)
...
def render_cuda_orthographic_gsplat(
    extrinsics: Float[Tensor, "batch 4 4"],
    width: Float[Tensor, " batch"],
    height: Float[Tensor, " batch"],
    near: Float[Tensor, " batch"],
