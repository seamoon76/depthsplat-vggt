...
if __name__ == '__main__':
    ...
    model_dir = os.path.join(args.dense_folder, 'with_gt')
    cam_dir = os.path.join(args.dense_folder, 'with_gt_cams')
    ...
    for i in range(num_images):
        ...
        for p3d_id in images[image_id].point3D_ids:
            ...
            transformed = np.matmul(extrinsic[image_id], [xyz[0], xyz[1], xyz[2], 1])
    ...
    id2idx = {image_id: idx for idx, image_id in enumerate(image_ids)}
    idx2id = {idx: image_id for idx, image_id in enumerate(image_ids)}
    ...
    def calc_score(inputs):
        ...
        image_id_i = idx2id[i]
        image_id_j = idx2id[j]
        id_i = images[image_id_i].point3D_ids
        id_j = images[image_id_j].point3D_ids
        id_intersect = [pid for pid in id_i if pid in id_j]

        cam_center_i = -np.matmul(extrinsic[image_id_i][:3, :3].T, extrinsic[image_id_i][:3, 3:4])[:, 0]
        cam_center_j = -np.matmul(extrinsic[image_id_j][:3, :3].T, extrinsic[image_id_j][:3, 3:4])[:, 0]
        ...
        for pid in id_intersect:
            ...
            sigma = args.sigma1 if theta <= args.theta0 else args.sigma2
            s += np.exp(-((theta - args.theta0) ** 2) / (2 * sigma ** 2))
        return i, j, s
    ...
    for i in range(num_images):
        image_id = idx2id[i]
        original_name = os.path.splitext(images[image_id].name)[0]
        cam_path = os.path.join(cam_dir, f'{original_name}_cam.txt')
        with open(cam_path, 'w') as f:
            ...
            for j in range(3):
                ...
                f.write('\n')
            d_min, d_interval, d_num, d_max = depth_ranges[image_id]
            f.write('\n%f %f %f %f\n' % (d_min, d_interval, d_num, d_max))
            
    with open(os.path.join(args.dense_folder, 'pair.txt'), 'w') as f:
        f.write('%d\n' % num_images)
        for i, sorted_score in enumerate(view_sel):
            f.write('%d\n%d ' % (i, len(sorted_score)))
            for ref_idx, s in sorted_score:
                f.write('%d %f ' % (ref_idx, s))
            f.write('\n')
            
        for i in range(num_images):
            image_id = idx2id[i]
            src_img_path = os.path.join(image_dir, images[image_id].name)
            original_name = os.path.splitext(images[image_id].name)[0]
            dst_img_path = os.path.join(renamed_dir, f'{original_name}.jpg')
